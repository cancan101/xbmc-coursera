...
def get_auth_url(className):
    return ("http://class.coursera.org/%s/auth/auth_redirector?type=login&"
            "subtype=normal&email=&visiting=&minimal=true") % className
...
def login_to_class(className, opener, username, password):
    ...
    if classLogin_title == "Coursera Login":
        ...
        data = urllib.urlencode({
            'email': username, 'password': password, 'login': 'Login'})
...
#       print heading_text
        ...
        for lecture_num, lecture in enumerate(lectures):
            ...
            if lecture_title is None:
                print("Unable to parse lecture in %s (lecture_title is "
                      "None)" % heading_text)
                continue
...
#           print "- %s (%s)" % (lecture_title_str, data_lecture_view_link)
            ...
            if resources is None:
                print "Unable to find resources for lecture %s in %s" % (
                    lecture_title_str, heading_text)
                continue
...
#               print "-- %s (%s) format=%s" % (title, href, resource_format)
            ...
            if not mp4_found:
                ...
                resources_entry["Lecture Video"] = grab_hidden_video_url(
                    data_lecture_view_link, opener)
