...
def isSettingsBad(username, password):
    return (username is None) or (password is None) or (
...
#       plugin.log.debug("one item case")
        return listCourseContents(
            courseShortName=getCourseShortName(courses[0]))
    ...
    for course in courses:
        ...
        if start_date_string is None or start_date_string == "":
            if course.get('start_year') is None and (
                    course.get('start_month') is None) and course.get(
                        'start_day') is None:
                start_date_string = "Self study"
            elif course.get('start_year') is not None and (
                    course.get('start_month') is not None) and course.get(
                        'start_day') is not None:
                date = datetime.date(year=course.get('start_year'),
                                     month=course.get('start_month'),
                                     day=course.get('start_day'))
                start_date_string = date.strftime("%d %B %Y")
            elif course.get('start_year') is not None and (
                    course.get('start_month') is not None):
                date = datetime.date(year=course.get('start_year'),
                                     month=course.get('start_month'))
...
def listCourses(shortName):
    ...
    for k, c in classes["topics"].iteritems():
        ...
        plugin.log.error("%s not found: %s" %
                         (shortName, [x["short_name"] for x in classes]))
...
def getClassCookies(className, username, password):
    ...
    cj.clear(domain="class.coursera.org", path="/%s" % className,
    ...
    for lecture_contents in sections.values():
        ...
        for section_name, section in sections.iteritems():
            if section["lecture_id"] == lecture_id:
                ...
                cookies = '&'.join(["%s=%s" % (x["name"], x["value"])
                                   for x in class_cookies])
...
def listLectureContents(courseShortName, section_num):
    ...
    if lecture_desired is None:
        plugin.log.error("Lecture %d for %s not found",
